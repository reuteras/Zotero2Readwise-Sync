...
class Readwise:
    ...
    def create_highlights(self, highlights: list[dict]) -> None:
        ...
        if resp.status_code != 200:
            error_log_file = f"error_log_{resp.status_code}_failed_post_request_
to_readwise.json"
    ...
    def convert_zotero_annotation_to_readwise_highlight(
        ...
        highlight_note = self.format_readwise_note(tags=annot.tags, comment=anno
t.comment)
        ...
        if annot.attachment_url is not None:
            ...
            highlight_url = f"zotero://open-pdf/library/items/{attachment_id}?pa
ge={location}%&annotation={annot_id}"
        return ReadwiseHighlight(
            ...
            category=(
                Category.articles.name if annot.document_type != "book" else Cat
egory.books.name
            ),
            highlighted_at=annot.annotated_at,
            source_url=annot.source_url,
            highlight_url=(annot.annotation_url if highlight_url is None else hi
ghlight_url),
            location=location,
        )

    def post_zotero_annotations_to_readwise(self, zotero_annotations: list[Zoter
oItem]) -> None:
        print(
            f"\nReadwise: Push {len(zotero_annotations)} Zotero annotations/note
s to Readwise...\n"
        ...
        for annot in zotero_annotations:
            try:
                if len(annot.text) >= 8191:
                    print(
                        f"A Zotero annotation from an item with {annot.title} (i
tem_key={annot.key} and "
                        f"version={annot.version}) cannot be uploaded since the
highlight/note is very long. "
...
self.failed_highlights)}) failed "
        ...
        finished_msg += (
            f"\n{len(rw_highlights)} highlights were successfully uploaded to Re
adwise.\n\n"
        )
        print(finished_msg)
...
")
        ...
        print(
            f"{len(self.failed_highlights)} highlights failed to format (hence f
ailed to upload to Readwise).\n"
