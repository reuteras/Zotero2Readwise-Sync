...
class ZoteroItem:
    ...
    def __post_init__(self):
        ...
        # Sample {'dc:relation': ['http://zotero.org/users/123/items/ABC', 'http
://zotero.org/users/123/items/DEF']}
        if self.relations:
    ...
    def format_author_list(authors):
        ...
        # Truncate each author name to 256 characters. this simplifies logic con
siderably
        ...
        # If the result is too long, remove authors from the end until it fits w
ith SEP + ET_AL appended
        # There is no risk that we will run out of authors for .pop() because MA
X_LENGTH >> MAX_AUTHOR_LENGTH
...
def get_zotero_client(
    ...
    ----------
    library_id: str
        If not passed, then it looks for `ZOTERO_LIBRARY_ID` in the environment
variables.
    api_key: str
        If not passed, then it looks for `ZOTERO_KEY` in the environment variabl
es.
    ...
    if library_id is None:
        try:
            ...
            raise ValueError(
                ...
                "You can set it as an environment variable `ZOTERO_LIBRARY_ID` o
r use `library_id` to set it."
            )
    ...
    if api_key is None:
        try:
            ...
            raise ValueError(
                ...
                "You can set it as an environment variable `ZOTERO_KEY` or use `
api_key` to set it."
            )
...
class ZoteroAnnotationsNotes:
    ...
    def get_item_metadata(self, annot: dict) -> dict:
        ...
        if "creators" in data:
            metadata["creators"] = [
                creator["firstName"] + " " + creator["lastName"] for creator in
data["creators"]
            ]
        if (
            "attachment" in top_item["links"]
            and top_item["links"]["attachment"]["attachmentType"] == "applicatio
n/pdf"
        ):
    ...
    def format_item(self, annot: dict) -> ZoteroItem:
        ...
        if item_type == "annotation":
            if annotation_type == "highlight":
                ...
                raise NotImplementedError("Handwritten annotations are not curre
ntly supported.")
        elif item_type == "note":
            text = data["note"]
            comment = ""
        ...
        for annot in annots:
            try:
                ...
                tag_condition = len(self.filter_tags) == 0 or any(
                    tag["tag"] in self.filter_tags for tag in annot["data"]["tag
s"]
                )
                if color_condition and tag_condition:
                    formatted_annots.append(self.format_item(annot))
            except Exception as e:
                self.failed_items.append({"item": annot, "reason": str(e)})
                continue

        finished_msg = "\nZOTERO: Formatting Zotero Items is completed!!\n\n"
        if self.failed_items:
            finished_msg += (
                f"\nNOTE: {len(self.failed_items)} Zotero annotations/notes (out
 of {len(annots)}) failed to format.\n"
                f"You can run `save_failed_items_to_json()` class method to save
 those items."
            )
        print(finished_msg)
